...
return new class extends Migration
{
    public function up(): void
    {
        Schema::create('apartment_payments', function (Blueprint $table) {
            $table->id();
            $table->string('idempotency_key')->nullable()->unique()->comment('offline-replay safety');
            ...
            $table->text('reason')->nullable()->comment('mandatory for refunds — enforced in ApartmentBillingService');
            $table->foreignId('ledger_id')->nullable()->constrained('apartment_ledgers')->nullOnDelete();
