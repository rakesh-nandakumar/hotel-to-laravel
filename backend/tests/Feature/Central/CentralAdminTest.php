...
it('lists platform operators', function () {
    ...
    $this->getJson('http://admin.localhost/api/central/admins')
        ->assertOk()
...
it('creates a platform operator', function () {
    ...
    $this->postJson('http://admin.localhost/api/central/admins', [
        'name' => 'New Operator',
...
it('updates a platform operator', function () {
    ...
    $this->putJson("http://admin.localhost/api/central/admins/{$admin->id}", [
        'name' => 'New Name',
...
it('deactivates a platform operator while another remains active', function () {
    ...
    $this->putJson("http://admin.localhost/api/central/admins/{$target->id}", [
        'is_active' => false,
    ])->assertOk();
...
it('keeps the platform reachable by never blocking the last active operator', function () {
    ...
    $this->putJson("http://admin.localhost/api/central/admins/{$self->id}", [
        'is_active' => false,
    ])->assertStatus(422);

    $this->deleteJson("http://admin.localhost/api/central/admins/{$self->id}")
        ->assertStatus(422);
});
...
it('deletes an operator who is not the last active one', function () {
    ...
    $this->deleteJson("http://admin.localhost/api/central/admins/{$target->id}")
        ->assertOk();
