...
import {
  Plus, KeyRound, MoreHorizontal, Eye, Pencil, Lock, Unlock, UserCheck, UserX, Ban, Trash2, Copy, Power, ShieldCheck, Search,
} from "lucide-react";
import { post, put, api } from "../lib/api";
import { useFetch } from "../lib/util";
import {
  Avatar, Badge, ConfirmDialog, Empty, ErrorText, Field, Modal, Pagination, Tabs, statusColor,
} from "../components/ui";
...
/**
 * "Staff & Access" — the User Management module (multi-role RBAC, per-user
 * permission overrides, role administration and an audit view), gated on
 * `user_management_users.access`. Roles that hold only `hotel_staff.set_pin`
 * (e.g. an Owner who just sets POS PINs) get the lightweight PIN-only picker.
 */
export default function StaffPage() {
  const { can } = usePermissions();
  return can("user_management_users.access") ? <UserManagement /> : <PinOnlyList />;
}

function UserManagement() {
  const { can } = usePermissions();
  const showRoles = can("user_management_roles.access");
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        {showRoles && (
          <Tabs
            tabs={[{ id: "users", label: "Users" }, { id: "roles", label: "Roles" }]}
            active={tab}
            onChange={setTab}
          />
        )}
      </div>
      {tab === "users" || !showRoles ? <UsersPanel /> : <RolesPanel />}
    </div>
  );
}
