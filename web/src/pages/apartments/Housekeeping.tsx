...
import { Badge, Empty, ErrorText, Modal, statusColor, Pagination } from "../../components/ui";
...
type Task = {
  ...
  status: { code: string };
  unit: { unit_no: string; status: { code: string }; unit_type: { name: string } | null };
  assigned_to?: { id: number; name: string } | null;
  checklist: { item: string; done: boolean }[];
};
type StaffLite = { id: number; name: string; roles: { name: string }[] };

export default function ApartmentHousekeeping() {
  const { can } = useAuth();
  const isManager = can("apartment_housekeeping.assign");
  const canChecklist = can("apartment_housekeeping.checklist");
  const canComplete = can("apartment_housekeeping.complete");
  const [page, setPage] = useState(1);
  const [pageSize, setPageSize] = useState(10);
  const { data, reload, error } = usePagedFetch<Task>(`/apartments/housekeeping/tasks?page=${page}&page_size=${pageSize}`, "tasks", [page, pageSize]);
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-extrabold">Apartment Housekeeping</h1>
      <p className="text-xs text-slate-500">A unit cannot be checked into/leased again until its cleaning checklist is submitted — completing a task marks the unit Available.</p>
          ...
          return (
            <button key={t.id} className="card p-4 text-left hover:shadow-md" onClick={() => setOpen(t)}>
              <div className="flex items-center justify-between">
                <span className="text-2xl font-black">{t.unit.unit_no}</span>
                <Badge color={statusColor(t.status.code)}>{t.status.code}</Badge>
              </div>
              <div className="text-xs text-slate-500">{t.unit.unit_type?.name ?? "—"} · created {fmtDateTime(t.created_at)}</div>
      ...
      {(tasks ?? []).length === 0 && <Empty text="No units waiting to be cleaned 🎉" />}
...
function ChecklistModal({ task, canAssign, canChecklist, canComplete, housekeepers, onClose }: { task: Task; canAssign: boolean; canChecklist: boolean; canComplete: boolean; housekeepers: StaffLite[]; onClose: () => void }) {
  ...
  const toggle = (i: number) => {
    ...
    setItems(next);
    put(`/apartments/housekeeping/tasks/${task.id}/checklist`, { checklist: next }).catch((e) => setError(e.message));
  };
  ...
  const complete = async () => {
    ...
    setError("");
    try {
      await post(`/apartments/housekeeping/tasks/${task.id}/complete`, { checklist: items });
      toast.success(`Unit ${task.unit.unit_no} is now Available`, "Cleaning checklist submitted");
      onClose();
    } catch (e) {
  ...
  return (
    <Modal open onClose={onClose} title={`${task.unit.unit_no} — cleaning checklist`}>
      {canAssign && (
        <div className="mb-3">
          ...
          <select
            ...
            onChange={(e) => put(`/apartments/housekeeping/tasks/${task.id}/assign`, { assigned_to_id: e.target.value || null }).catch((err) => setError(err.message))}
          >
      ...
      {canComplete && (
        <button className="btn-primary mt-4 w-full !py-3" disabled={!allDone || busy} onClick={complete}>
          {allDone ? "Submit checklist — unit becomes Available" : `Complete all ${items.length} items first`}
        </button>
      )}
    </Modal>
  );
